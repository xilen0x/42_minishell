...
int	builtin_export(t_cmd *cmd, t_env env)
{
	...
	else if (ca_strchr(*cmd->command_and_arg, '='))
	{
		//builtin_export_with_arg(cmd, &env);// si viene por ej. 'export TEST=10'
		add_one_arr2d(cmd->command_and_arg, cmd->command_and_arg[1]);
	}
	return (0);
}
